...
import (
	...
	model "github.com/54m/how-to-gen-tools/back/domain/model"
)

//go:generate ../../bin/mockgen -source $GOFILE -destination mock/mock_user.go
...
func (repo *userRepository) search(v interface{}, param *UserSearchParam, q *firestore.Query) ([]*model.User, error) {
	...
	if q == nil {
		...
		if param.Age != nil {
			for _, chain := range param.Age.QueryGroup {
				query = query.Where("Age", chain.Operator, chain.Value)
			}
			if direction := param.Age.OrderByDirection; direction > 0 {
				query = query.OrderBy("Age", direction)
				query = param.Age.BuildCursorQuery(query)
			}
		}
