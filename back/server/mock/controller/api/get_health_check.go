...
func (ctrl *getHealthCheckController) GetHealthCheck(
	...
	if ago != "" {
		if err := json.Unmarshal([]byte(ago), option); err != nil {
			log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/api/health_check): %+v", err)
	...
	err = fs.WalkDir(mock.MockJSONFS, "json/api/get_health_check", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
