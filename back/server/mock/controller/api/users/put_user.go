...
func NewPutUserController(cp interface{}) PutUserController {
...
// @Accept json
// @Produce json
// @Param body body types.PutUserRequest true "request parameter"
// @Param user_id path string true "userID"
// @Success 200 {object} types.PutUserResponse
...
func (ctrl *putUserController) PutUser(
	...
	type Mock struct {
		Meta struct {
			...
			MatchRequest *types.PutUserRequest `json:"match_request"`
		} `json:"meta"`
		Payload interface{} `json:"payload"`
	}

	jsons := make(map[string]*Mock)
	err = fs.WalkDir(mock.MockJSONFS, "json/api/users/put_user", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
