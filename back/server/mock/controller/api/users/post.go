...
// NewPostController ...
func NewPostController(cp interface{}) PostController {
...
// @Accept json
// @Produce json
// @Param body body types.PostRequest true "request parameter"
...
func (ctrl *postController) Post(
	...
	if ago != "" {
		if err := json.Unmarshal([]byte(ago), option); err != nil {
			log.Printf("failed to JSON Unmarshal to `Api-Gen-Option` header(/api/users/): %+v", err)
	...
	type Mock struct {
		Meta struct {
			...
			MatchRequest *types.PostRequest `json:"match_request"`
		} `json:"meta"`
		Payload interface{} `json:"payload"`
	}

	jsons := make(map[string]*Mock)
	err = fs.WalkDir(mock.MockJSONFS, "json/api/users/post_", fs.WalkDirFunc(func(path string, info fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
